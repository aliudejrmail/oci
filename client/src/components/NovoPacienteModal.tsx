...
interface NovoPacienteModalProps {
  open: boolean
  onClose: () => void
  onSuccess?: (paciente: { id: string; nome: string; cpf: string }) => void
}
...
export default function NovoPacienteModal({ open, onClose, onSuccess }: NovoPacienteModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess?.({ id: data.id, nome: data.nome, cpf: data.cpf })
      onClose()
    } catch (err: any) {
      ...
      >
        <div className="flex items-center justify-between p-2 border-b border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-gray-900">Cadastrar Novo Paciente</h2>
          </div>
